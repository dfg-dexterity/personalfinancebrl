...
export function CategoryPickerModal() {
  const { picker, data, categorize, closePicker } = useFinance()
  if (picker == null || !data) return null

  const tx = data.transactions.find((t) => t.id === picker)
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mb-[18px] text-[19px] font-extrabold text-ink">Escolher categoria</div>
          ...
          {PICKER_CATEGORIES.map((k) => {
            ...
            return (
              <button
                key={k}
                type="button"
                onClick={() => void categorize(picker, k)}
