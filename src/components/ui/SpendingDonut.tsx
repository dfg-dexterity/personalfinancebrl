import type { ApiReportCat } from '../../api/types'
  ...
  return (
    <div className="flex items-center" style={{ gap: wrapGap }}>
      ...
      <div className="flex flex-1 flex-col" style={{ gap: legendGap }}>
        {cats.length === 0 && <span className="text-[13px] text-muted">Sem gastos no mês.</span>}
        {cats.map((c) => (
          <div key={c.name} className="flex items-center gap-[9px] text-[13px]">
            <span className="rounded-full" style={{ width: dot, height: dot, background: c.color }} />
